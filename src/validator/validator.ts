...
import type { RunMode, RunStatus } from '../models/types.js';
...
export class ArtifactPresenceValidator implements Validator {
  ...
  async validate(input: ValidatorInput): Promise<ValidatorDecision> {
    ...
    if (missing.length > 0) {
      ...
      return { status: 'BLOCK', notes, exitCode: 1 };
    }
    ...
    if (input.mode === 'baseline') {
      ...
      return { status: 'BASELINE_BLOCKED_EXPECTED', notes, exitCode: 0 };
    }
    ...
    return { status: 'PASS', notes, exitCode: 0 };
  }
}
